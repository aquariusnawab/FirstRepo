...
Class Allocator
	...
	Private Function GetRunInfo(objTestSet)
		...
		For i = lstTSTests.Count To 1 Step -1
			If IsFlaggedForExecution(lstTSTests.Item(i).Test.Field("TS_SUBJECT").Name,_
											lstTSTests.Item(i).Test.Name, arrTestParameters) Then
	...
	'###################################################################################################################
	Private Function IsFlaggedForExecution(strTestScenario, strTestCase, arrTestParameters)
	...
	Private Sub SetTestParameters(objCurrentTSTest, arrTestParameters)
		...
		For Each objParam in lstParams
			Select Case objParam.Name
				Case "IterationMode":
					objParam.ActualValue = arrTestParameters(4, 0)
				Case "StartIteration":
	...
	Private Sub UpdateTestBatchStatus(lstTSTests)
		Dim strTestScenario, strTestCase, strTestDescription, strExecutionTime, strTestStatus
		Dim i
		
		For i = 1 to lstTSTests.Count
			strTestScenario = lstTSTests.Item(i).Test.Field("TS_SUBJECT").Name
			strTestCase = lstTSTests.Item(i).Test.Name
			m_objQcConnection.IgnoreHTMLFormat = True
			strTestDescription = lstTSTests.Item(i).Test.Field("TS_DESCRIPTION")
			...
			gobjReport.UpdateResultSummary strTestScenario, strTestCase, strTestDescription, strExecutionTime, strTestStatus
		Next
	End Sub
	'###################################################################################################################
	
	'###################################################################################################################
	'Function Description	: Function to do wrapup activities after completing the test batch execution
	'Input Parameters 		: None
	'Return Value    		: None
	'Author					: Cognizant
